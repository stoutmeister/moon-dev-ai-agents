...
from agents.wholesale_dialer_agent import (
...
def _boot_dialer():
    ...
    from agents.wholesale_dialer_agent import AI_MODEL_TYPE, AI_MODEL_NAME

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    dialer.type = "wholesale_dialer_webhook"
    dialer.start_time = datetime.now()
    dialer.model = get_claude_model(AI_MODEL_NAME)
    if not dialer.model:
...
def _get_lead_context(call_sid, to_number):
    ...
    from agents.wholesale_dialer_agent import TRACED_CSV
...
if __name__ == "__main__":
    cprint("\n🌙 Wholesale Dialer Webhook starting... 📞🏠", "cyan")
    _boot_dialer()
